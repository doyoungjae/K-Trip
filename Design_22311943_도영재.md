...
| :--- | :--- | :--- | :--- |
| 2026-05-30 | v1.0.0 | K-Trip 개념정의서 및 분석서 기반 아키텍처 및 시스템 설계서 초안 작성 | 도영재 |
| 2026-06-04 | v1.0.1 | 각 Use Case별 시퀀스 다이어그램 및 상태 머신 다이어그램 구현, 관련 설명 명세 기술  | 도영재 |
| 2026-06-04 | v1.0.2 | 클래스 다이어그램 구현 및 명세서 작성 | 도영재 |
...
2. **클래스 다이어그램 (Class Diagram)**
   - 2.1 Class Diagram
   - 2.2. CORE 레이어 상세 명세 (Business Logic & Data Engine)
   - 2.2.1. Repository (Interface)
   - 2.2.2 TravelLogProcessor
   - 2.2.3. SupabaseClient / LocalCacheStorage
   - 2.3. 데이터 처리 로직 및 엔티티 상세 명세 (Logic & Entities)
   - 2.3.1. GpsLogClusteringFilter
   - 2.3.2. MediaMetadataExtractor
   - 2.4. VIEW 레이어 상세 명세 (UI Components)
   - 2.4.1. BaseView
   - 2.4.2. TravelLogView
...
classDiagram
    ...
    class TravelLogProcessor {
        -GpsLogClusteringFilter filter
    ...
    %% [데이터 로직 및 엔티티]
    class GpsLogClusteringFilter {
        +cluster(logs: List~LatLng~)
    }
    ...
    class TravelLogView {
        -TravelLogProcessor processor
        +captureEvent()
    }

    %% [관계 표현 (강의 노트 기법 적용)]
    BaseView <|-- TravelLogView : Inheritance
    TravelLogView --> TravelLogProcessor : Association
    
    TravelLogProcessor *-- GpsLogClusteringFilter : Composition
    TravelLogProcessor *-- MediaMetadataExtractor : Composition
    
    TravelLogProcessor ..> Repository : Dependency
    Repository <|.. SupabaseClient : Realization
    Repository <|.. LocalCacheStorage : Realization
 ```

## 2.2. CORE 레이어 상세 명세 (Business Logic & Data Engine)

### 2.2.1. Repository (Interface)
**Class Description**: 데이터 영속성 계층의 최상위 추상화 인터페이스. 특정 저장소에 의존하지 않는 표준 명세를 정의함.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| syncLocalCache | self | None | instance | public | 로컬 캐시와 원격 데이터 동기화 |
| pushToSupabase | data | bool | instance | public | 정제된 데이터를 원격 서버로 전송 |

### 2.2.2. TravelLogProcessor
**Class Description**: 여행 기록 가공의 핵심 비즈니스 로직 컨트롤러.

**Attributes**
| Name | Type | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- |
| __filter | GpsLogClusteringFilter | instance | private | 클러스터링 필터 객체 |
| __extractor | MediaMetadataExtractor | instance | private | 메타데이터 추출 객체 |

**Operations**
| Name | Argument | Returns | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| runClusteringFilter | logs | List | instance | public | 방문지 자동 분할 알고리즘 실행 |
| encodeVideo | data | File | instance | public | 로그를 기반으로 비디오 인코딩 |

### 2.2.3. SupabaseClient / LocalCacheStorage
**Class Description**: `Repository`를 구현하는 구체 클래스. 인프라 통신 및 물리적 파일 저장 담당.

**Attributes**
| Name | Type | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- |
| __db_endpoint | String | instance | private | 서버 엔드포인트 주소 |

**Operations**
| Name | Argument | Returns | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| upsert | data | bool | instance | public | 서버 데이터 업데이트 및 삽입 |
| writeLog | data | None | instance | public | 로컬 파일 시스템에 로그 저장 |
...
### 2.3.1. GpsLogClusteringFilter
**Class Description**: 위치 데이터 분석 유틸리티. GPS 로그를 분석하여 체류지를 판별함.

**Attributes**
| Name | Type | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- |
| __threshold | float | instance | private | 방문지 판별 임계값 |

**Operations**
| Name | Argument | Returns | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| cluster | logs | List | instance | public | 위경도 기반 방문지 클러스터링 |

### 2.3.2. MediaMetadataExtractor
**Class Description**: 미디어 파일에서 EXIF 데이터를 추출하는 파싱 유틸리티.

**Attributes**
| Name | Type | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- |
| __format | String | instance | private | 지원하는 파일 포맷 목록 |

**Operations**
| Name | Argument | Returns | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| parseExif | file | Object | instance | public | 파일 내 메타 정보 파싱 및 변환 |
...
### 2.4.1. BaseView
**Class Description**: UI 렌더링 생명주기를 정의하는 추상 부모 클래스.

**Attributes**
| Name | Type | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- |
| __context | BuildContext | instance | private | UI 빌드 컨텍스트 |

**Operations**
| Name | Argument | Returns | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| updateUiState | self | None | instance | public | 데이터 변경에 따른 화면 갱신 |

### 2.4.2. TravelLogView
**Class Description**: 여행 기록을 시각화하고 사용자 입력을 처리하는 화면 클래스.

**Attributes**
| Name | Type | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- |
| __processor | TravelLogProcessor | instance | private | 로직 처리를 위한 컨트롤러 객체 |

**Operations**
| Name | Argument | Returns | Scope | Visibility | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| captureEvent | self | None | instance | public | 사용자 캡처 이벤트 처리 및 엔진 호출 |
---
