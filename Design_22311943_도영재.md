...
| 2026-06-04 | v1.0.2 | 클래스 다이어그램 구현 ) | 도영재 |
...
2. **클래스 다이어그램 (Class Diagram)**
   - 2.1. CORE 레이어 (비즈니스 로직 및 데이터 엔진)
   - 2.1.1. Repository (Interface)
   - 2.1.2 TravelLogProcessor
   - 2.1.3. SupabaseClient / LocalCacheStorage
   - 2.2 데이터 처리 로직 및 엔티티 (Logic & Entities)
   - 2.2.1 GpsLogClusteringFilter
   - 2.2.2 MediaMetadataExtractor
   - 2.3 VIEW 레이어 (UI 컴포넌트)
   - 2.3.1. BaseView (Abstract)
   - 2.3.2. TravelLogView
...
### 2.1.1. Repository (Interface)
- **역할**: 데이터 영속성 계층의 최상위 추상화 인터페이스.
- **책임**: 특정 저장소(로컬 혹은 클라우드)에 의존하지 않고 데이터를 저장/동기화하기 위한 표준 명세를 정의합니다.
- **주요 메서드**:
  - `syncLocalCache()`: 로컬 데이터와 원격 데이터의 상태를 일치시킵니다.
  - `pushToSupabase()`: 정제된 로그 데이터를 원격 서버로 전송합니다.

### 2.1.2 TravelLogProcessor
- **역할**: 여행 기록 가공의 핵심 비즈니스 로직 컨트롤러.
- **책임**: GPS 로그 클러스터링 및 미디어 메타데이터 추출을 관리하며, 이를 기반으로 숏폼 비디오 인코딩 로직을 수행합니다.
- **주요 메서드**:
  - `runClusteringFilter()`: 방문지 자동 분할 알고리즘을 실행합니다.
  - `encodeVideo()`: 여행 로그를 기반으로 멀티미디어 파일을 생성합니다.

### 2.1.3. SupabaseClient / LocalCacheStorage
- **역할**: `Repository` 인터페이스를 구현(Realization)하는 구체 클래스.
- **책임**: `SupabaseClient`는 클라우드 API를 통한 데이터 입출력을, `LocalCacheStorage`는 단말 내부 파일 시스템에 로그를 기록하는 물리적 구현을 담당합니다.
...
### 2.2.1 GpsLogClusteringFilter
- **역할**: 위치 데이터 분석 유틸리티.
- **책임**: 시계열 GPS 데이터를 분석하여 특정 지역 내 체류 여부를 판별하고, 클러스터링(방문지 분할) 알고리즘을 적용합니다.
- **주요 메서드**:
  - `cluster(logs)`: 입력된 위경도 로그 리스트를 기반으로 유의미한 이동 경로와 방문지를 그룹화합니다.

### 2.2.2 MediaMetadataExtractor
- **역할**: 미디어 데이터 파싱 유틸리티.
- **책임**: 이미지/영상 파일에서 EXIF 데이터를 추출하여 촬영 시간과 위치 정보를 정형화합니다.
- **주요 메서드**:
  - `parseExif(file)`: 파일 시스템의 원본 데이터에서 메타 정보를 파싱하여 객체로 변환합니다.
...
### 2.3.1. BaseView (Abstract)
- **역할**: 모든 화면 클래스의 추상 부모 클래스.
- **책임**: UI 렌더링 생명주기를 강제하고 상태 갱신 로직(`updateUiState`)을 규격화합니다.

### 2.3.2. TravelLogView
- **역할**: 여행 기록을 시각화하고 사용자 입력을 받는 화면.
- **책임**: `TravelLogProcessor`와 협력하여 사용자의 이벤트(로그 캡처 등)를 엔진에 전달합니다.
- **주요 메서드**:
  - `captureEvent()`: 사용자가 화면 상에서 발생시키는 기록 관련 인터랙션을 감지하고 컨트롤러에 전달합니다.
